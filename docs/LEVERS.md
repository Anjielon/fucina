...
| **Channel reordering (SSR)** | permute columns so similar/hard channels cluster into the same superblocks, letting a shared scale serve them better. **Pure pre-processing — needs no format change and can go in the forge today.** Authors: +11.6 points average accuracy over PB-LLM at comparable bit width | 🧪🔥 highest-value implementable lever left | PT²-LLM, arXiv 2510.03267 |
