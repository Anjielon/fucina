...
| **down_proj above gate/up** | three independent sources agree; the mechanism is now clear — SwiGLU concentrates outliers in down's *input*, and the "super weights" whose removal collapses a model live in `down_proj` | ✅ | arXiv 2411.07191; Unsloth: "quantizing down_proj might not be a good idea, especially in early layers" |
| **Diagonal imatrix weighting for the down fit** | the importance matrix already stores a per-expert diagonal second moment for `ffn_down_exps` — no capture needed. Using it to weight the block-scale search: **no benefit** (−0.02, +0.23, +0.06 output error on three experts). The risk in `down` is concentrated in a few channels, and weighting a per-block scale is too blunt an instrument for that | ⛔ measured, on a simplified fit | ours |
...
| **Sparse outlier rescue (top-k weights at f16)** | top 1% of our weights carry 18% of the ternary error. **But the field's rule of thumb is that <1% at f16 cuts up to 75%** — so ternary error is markedly *less* concentrated than the 4-bit regime, being dominated by three-level rounding rather than sparse spikes. And GGUF has no sparse side-channel: ggml's quantization author prototyped per-superblock outlier slots in 2023, called them "far from perfect", and declined the proper version because it "would require a new tensor type along with a new ggml operation" and would hurt batched inference | ⛔ not implementable in GGUF; the rejection reason (batching) does not apply to single-user serving, so it remains open for a custom path | ours; SpQR arXiv 2306.03078; llama.cpp discussion #1595 |
| **Channel reordering** | mechanically the cleanest lever available — for `down` the permuted axis is the expert's *private* intermediate, so no shared activation to coordinate, no engine change, no format change. The literature is encouraging: GPTQ's act-order alone takes OPT-66B from 14.16 to 9.95 perplexity at 3-bit, and the gain grows as bits fall. **Measured on our tensors before building it: it does not pay.** Sorting columns by magnitude or variance buys between **0.006 and 0.46 points out of 44** — under 1% relative — and the effect is largest on `up`, which already works, and essentially zero on `down`, which is the projection that hurts. The control settles it: a *random* permutation scores 43.908% against 43.900% for the original order, so there is no column structure here to exploit | ⛔ measured, does not apply to these tensors | ours; GPTQ act-order; PT²-LLM arXiv 2510.03267 |
| **Channel reordering, `up`/`gate` input side** | their input is the shared residual stream, so any permutation must be identical across every expert *and* the router. The clean mechanism is `g_idx` indirection — physical order untouched, a per-channel group id gathered at dequant — which **GGUF has nowhere to store**. The alternative is folding the permutation into the preceding norm and every consumer of the residual stream: possible offline, but a large mechanical change | ⛔ blocked by the format, unless done as a whole-layer offline transform | RPTQ arXiv 2304.01089 |
| **Sort key: magnitude beats clustering** | published ablation: random 89.0 → variance sort 83.3 → magnitude sort 85.3 → elaborate joint criterion 80.4. A plain sort captures most of the gain at O(n log n); similarity clustering buys a few points more | ✅ start simple | PermuQuant arXiv 2605.09503; CodeQuant arXiv 2604.10496 |
| **Residual-coded outliers instead of f16** | for the salient weights, encode a *binary residual* on top of the ternary value rather than restoring full precision — a fraction of the 0.16 bits/weight an f16 rescue costs. Conceptually our second plane, made sparse | 🧪 v4 | BiLLM, arXiv 2402.04291 |
