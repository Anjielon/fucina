...
| **Channel reordering, `down` only** | permute each expert's *private* intermediate dimension so its 256-blocks are homogeneous, folding the inverse into that same expert's `up`/`gate` output rows. **Entirely local to one expert: no shared activation to coordinate, no engine change, no format change.** And `down` is the projection that hurts us. Evidence that this matters most at low bit: GPTQ's act-order alone takes OPT-66B from 14.16 to 9.95 perplexity at 3-bit, and the gain grows as bits fall | 🧪🔥 **the implementable lever** | GPTQ act-order; PT²-LLM arXiv 2510.03267; RPTQ arXiv 2304.01089 |
